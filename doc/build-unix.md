# Unix Build of Firebug Coin Core
...
You must install required dependencies to build a basic Firebug Coin daemon, optional dependencies may vary according to your requirements.
...
Firebug Coin Core's released binaries are built using the "depends system", which
...
**Optional dependencies** :
```bash
# Qt (required for apterus-qt GUI)
...
According to installed dependencies, the following steps will compile `apterusd`, `apterus-cli` and `apterus-qt`.
...
#### Enable Qt GUI
Create `apterus-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
...
By default, **Firebug Coin Core expects BerkeleyDB 5.3**.
...
Otherwise, you can build Firebug Coin Core from self-compiled [depends](/depends/README.md).

**Note**: You only need Berkeley DB if the wallet is enabled (see [*Disable-wallet mode*](#disable-wallet-mode)).

#### Disable-wallet mode
When the intention is to run only a P2P node without a wallet, Firebug Coin may be compiled in
...
#### Security
To help make your Firebug Coin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, Firebug Coin should be built with a non-executable stack,
...
least 1.5 GB of memory available when compiling Firebug Coin Core.
