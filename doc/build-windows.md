...
Below are some notes on how to build Firebug Coin Core for Windows.
...
Note that for WSL v1 the Firebug Coin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/lebowskiscoin, AND not under, for example, /mnt/d/lebowskiscoin. 
...
way. This will install to `c:\workspace\lebowskiscoin`, for example:

    make install DESTDIR=/mnt/c/workspace/lebowskiscoin
