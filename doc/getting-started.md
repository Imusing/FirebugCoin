...
This tutorial will help you to go through the basics to use Firebug Coin Core after you completed the [installation instructions](/INSTALL.md). You now have `apterusd` or `apterus-qt` executables available to run a node, and `apterus-cli`/`lebowskiscoin-tx` tools to help you transact DOGE.
...
> ```console
> shibetoshi:~$ /path/to/apterus-cli [arguments ...]
> ```

### Table of contents

1. [Starting a lebowskiscoin node](#starting-a-lebowskiscoin-node)
...
## Starting a Firebug Coin node

To start your node, you can run an headless server using `apterusd`:
```console
shibetoshi:~$ apterusd -daemon
```

Or you can use the Graphical User Interface (GUI), `apterus-qt`:
```console
shibetoshi:~$ apterus-qt
```
...
Your node is now running and starts with a *synchronization process* that downloads the entire blockchain from other nodes. This operation will take many hours to complete, but you are now part of the Firebug Coin network!

> **Note:** The rest of this guide assumes the use of an headless node. The RPC server is not exposed with `apterus-qt` until you activate the `-server` option as a startup argument, but inside the GUI application, you can use all the commands explored below (without `apterus-cli`) by going to `Help -> Debug window` and inside the popup window selecting the tab `Console`.

## Introduction to the JSON-RPC API

Firebug Coin Core exposes a JSON-RPC interface that allows you to request information about the network, blockchain and individual transactions, send transactions to the networks and manage your wallet.

The Firebug Coin Core installation provides the `apterus-cli` tool to interact with the JSON-RPC from the command line, and the interface is exposed over HTTP on port `22555`, so that other tools and libraries can interact with it.

To have an overview of the available commands, use the `help` command:

```console
#List all commands
shibetoshi:~$ apterus-cli help

#Get help for a specific command
shibetoshi:~$ apterus-cli help COMMAND
```
...
By default, the Firebug Coin Core software will automatically create an address for you and securely store the private key in the wallet file.

You can list wallet addresses using `getaddressesbyaccount`:

```console
shibetoshi:~$ apterus-cli getaddressesbyaccount ""
[
...
```console
shibetoshi:~$ apterus-cli getnewaddress
...
```console
shibetoshi:~$ apterus-cli backupwallet /path/of/wallet/backup
```

**Tip:** Firebug Coin addresses start with the letter `D`.
...
```console
#Syntax
shibetoshi:~$ apterus-cli getbalance "*" minconf
```
...
```console
#Syntax
shibetoshi:~$ apterus-cli sendtoaddress address amount

#Example
shibetoshi:~$ apterus-cli sendtoaddress nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh 420
```
...
```console
#Syntax
shibetoshi:~$ apterus-cli listunspent minconf maxconf '["address", ...]'

#Example
shibetoshi:~$ apterus-cli listunspent 1 9999999 '["nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P"]'
[
  {
...
shibetoshi:~$ apterus-cli createrawtransaction "$utxos_to_use" '{"address":amount, ...}'
...
shibetoshi:~$ apterus-cli createrawtransaction "$utxos_to_use" '{"nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh":69, "nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P": 30.999}'
...
> **Tip:** The transaction returned is encoded in hexadecimal encoding. You can use `apterus-cli decoderawtransaction` or `lebowskiscoin-tx -json` to convert the content to JSON format.

##### signrawtransaction

Before sending a transaction, it must be signed by the private key that the address was derived from. Firebug Coin Core will automatically use the correct private key when spending UTXO known to the wallet.

```console
#Syntax
shibetoshi:~$ apterus-cli signrawtransaction encoded_transaction

#Example
shibetoshi:~$ apterus-cli signrawtransaction "01000000013324faf8f03695261611669f5bdd93b68a86cf02db12dc46642ed50666ed69b80000000000ffffffff0200a5459b010000001976a91418a89ee36293f15c4db4c01173babd579243161188ac60b8c4b8000000001976a914c6977da37560e1432c2e14e16952981a4c272cac88ac00000000"
{
...
```console
#Syntax
shibetoshi:~$ apterus-cli sendrawtransaction signed_transaction

#Example
shibetoshi:~$ apterus-cli sendrawtransaction 
...
> **Note:** To be able to query transactions not related to your own wallet, like in this example, you will need to enable the `-txindex` option. This options requires the Firebug Coin Core software to re-index the entire blockchain, and can take up to several hours.

First, request the information about block 69:

```console
#Find block hash from his height
shibetoshi:~$ apterus-cli getblockhash 69
3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b

#Get block data
shibetoshi:~$ apterus-cli getblock 3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b
{
...
```console
#Syntax
shibetoshi:~$ apterus-cli getrawtransaction txid verbose

#Example
shibetoshi:~$ apterus-cli getrawtransaction 695ce4208fa7a87ef9e99805b0910dc129058ecdceb5cef7e25f71dcdc7936db 1
{
...
Using `apterusd -help` will display all available configuration parameters that can be added as arguments:

**Command example :**
```console
shibetoshi:~$ apterusd -daemon -paytxfee=0.01 -sendfreetransactions=1 -maxconnections=150
```

Configuration can be persisted by creating a `lebowskiscoin.conf` file. Create it in the directory defined with the `datadir` setting, `$HOME/.lebowskiscoin` by default, or specify the file location with `-conf`.

**lebowskiscoin.conf example :**
```
daemon=1
server=1
listen=1
...
maxconnections=150
```
You can see a more concrete example [here](/contrib/debian/examples/lebowskiscoin.conf).

### Mainnet, testnet and regtest

When trying out new things, for example to test your application that interacts with the Firebug Coin chain, it is recommended to not use the main Firebug Coin network. Multiple networks are built-in for this purpose.
...
When not specifying any network, *Mainnet* is the network used by default. To enable *testnet*, use the `apterusd -testnet`.

To enable *regtest*, use the `-regtest` option.

> **Tip:** Remember to specify the network when you want to use `apterus-cli`.

### Data directory

The data directory is the location where Firebug Coin Core files are stored, including the wallet, log files and blocks. You can modify the location with the `-datadir` setting.

**Default location :**

Platform | Data directory path
---------|--------------------
Linux    | `$HOME/.lebowskiscoin`
macOS    | `$HOME/Library/Application Support/Firebug Coin`
Windows  | `%APPDATA%\Firebug Coin`

You may need to specify `-datadir` also when using `apterus-cli`.

See the [full documentation on file system](files.md) for more information.

### RPC credentials

Authentication is required to interact with the RPC interface. When no credentials are provided, Firebug Coin uses a [random cookie](https://bitcoin.org/en/release/v0.12.0#rpc-random-cookie-rpc-authentication) that gets generated when the software is launched. It's possible to define your own credentials using `rpcuser` and `rpcpassword` parameters.
...
Running Firebug Coin Core can require a lot of memory, so in some situations it may be necessary to optimize its usage. You can find more information about reducing the memory footprint in the [related guide](reduce-memory.md).

### Troubleshooting

By default, Firebug Coin Core keeps detailed logs in the `debug.log` file, located in the `datadir`. Alternatively, the `-printtoconsole` parameter displays the log interactively to the terminal instead.
