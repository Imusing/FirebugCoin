...
The data directory is the default location where the Firebug Coin Core files are stored.
...
macOS    | `$HOME/Library/Application Support/FirebugCoin/`
Windows  | `%APPDATA%\Firebug Coin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for the `lebowskiscoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`./`               | `lebowskiscoin.conf`       | User-defined configuration settings for `apterusd` or `apterus-qt`; can be specified by `-conf` option
`./`               | `apterusd.pid`        | Stores the process ID (PID) of `apterusd` or `apterus-qt` while running; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `apterusd` or `apterus-qt`
...
`apterus-qt` uses the [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Firebug Coin Core:
