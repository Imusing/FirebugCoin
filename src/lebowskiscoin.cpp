// Copyright (c) 2015-2021 The Firebug Coin Core developers
...
// FirebugCoin: Normally minimum difficulty blocks can only occur in between
...
unsigned int CalculateDogecoinNextWorkRequired(bool fNewDifficultyProtocol, const int64_t nTargetTimespanCurrent, const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
    ...
    // FIREBUG COIN: 1 in 10000 chance for every block to be a minimum difficulty block, to keep the chain moving in case of low hashrate.
    // that is a 0.01% chance! good luck finding one of these blocks! (and if you do, you deserve it (sorry if it was a burn block that loses you money))
    if (generateMTRandom(pindexLast->nHeight, 10000) == 1)
        bnNew = bnPowLimit;
...
bool CheckAuxPowProofOfWork(const CBlockHeader& block, const Consensus::Params& params)
{
    ...
    if (!block.auxpow) {
        ...
        // We have to patch this because FirebugCoin genesis block is invalid.
...
CAmount GetDogecoinBlockSubsidy(int nHeight, CAmount nFees, const Consensus::Params& consensusParams, uint256 prevHash)
{
    ...
    // Randomly give out bigger rewards
    ...
    // welcome to firebug coin, where the chaos happens...
    // possible events:
    // 50% chance of normal 20 APTERUS reward
    // 30% chance of 2x reward
    // 5% chance of 10x reward
    // 1% chance of -- reward (negative reward, i.e. burn)
    // 1% chance of 100x reward
    // 0.01% chance of -10x reward (negative reward, i.e. burn)
    ...
    if (rand < 50000)
        ...
        nSubsidy = 2000 * COIN;     // 9% ish
    else
        nSubsidy = -200 * COIN;     // 0.01% ish
