...
#if defined(NDEBUG)
# error "FirebugCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "FirebugCoin Signed Message:\n";
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // lebowskiscoin: Switch maturity at digishield activation
...
void ThreadScriptCheck() {
    RenameThread("lebowskiscoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // lebowskiscoin: BIP30 has been active since inception
    ...
    // lebowskiscoin: BIP16 has been enabled since inception
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // lebowskiscoin: Disallow AuxPow blocks before it is activated.
