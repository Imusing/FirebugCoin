...
const char * const BITCOIN_CONF_FILENAME = "lebowskiscoin.conf";
const char * const BITCOIN_PID_FILENAME = "apterusd.pid";
...
#else
    const char* pszModule = "lebowskiscoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FirebugCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FirebugCoin";
#else
    // Unix
    return pathRet / ".firebugcoin";
#endif
#endif
}
