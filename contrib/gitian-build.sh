...
SIGNER=
VERSION=
url=https://github.com/lebowskiscoin/lebowskiscoin
proc=2
mem=2000
...
Standalone script to perform the gitian build of Firebug Coin Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/lebowskiscoin/lebowskiscoin
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/lebowskiscoin-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    git clone https://github.com/lebowskiscoin/gitian.sigs.git
    git clone https://github.com/lebowskiscoin/lebowskiscoin-detached-sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/lebowskiscoin-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean lebowskiscoin git directory because of old caching
    if [ -d inputs/lebowskiscoin/ ]; then
        echo "Cleaning Firebug Coin directory..."
        rm -rf inputs/lebowskiscoin/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit lebowskiscoin="$COMMIT" --url lebowskiscoin="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
