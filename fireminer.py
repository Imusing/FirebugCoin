# Modifed from original code at https://github.com/vecocoin/vecominer/blob/main/VecoMiner.py
# run qt with -server -rpcuser=a -rpcpassword=b to start the miner
import json
import requests
import argparse
import time
...
def parse_args():
    parser = argparse.ArgumentParser(description="FirebugCoin CLI Miner")

    parser.add_argument("-u", "--username", type=str, default="a", help="RPC username (default: a)")
    parser.add_argument("-p", "--password", type=str, default="b", help="RPC password (default: b)")
    parser.add_argument("--host", type=str, default="127.0.0.1", help="RPC server host (default: 127.0.0.1)")
    parser.add_argument("--port", type=int, default=16778, help="RPC server port (default: 16778)")
    ...
    parser.add_argument("-t", "--threads", type=int, default=multiprocessing.cpu_count() // 4,
                        help="Number of mining threads (default: " + str(multiprocessing.cpu_count() // 4) + ")")
...
if __name__ == "__main__":
    ...
    print(f"🚀 Starting APTERUS Miner with {args.threads} threads...")
